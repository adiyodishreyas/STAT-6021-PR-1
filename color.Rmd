...
# Analysis of the Color of a Diamond
...
We did a similar analysis for color and cut, where we grouped the data set by cut and then tried to assess the impact of color on each individual group. The box plots in this case didn't show strong evidence that colorless diamond should cost higher than the more colored ones. This maybe because variables other than cut and color might be influencing the price.

```{r, echo=FALSE}
ggplot(d, aes(color, log10(price), fill=color))+
  geom_boxplot()+
  facet_wrap(~cut)
  labs(x="Color", y="log10(Price)", title="log10(Price) vs Color grouped by cut")
```
Then to find a correlation between color and cut we also created proportion bar chart. This chart will help us answer questions such as does a particular cut type occur more often for a particular color in our data set?
...
We did a similar analysis for color and clarity, where we grouped the data set by clarity and then tried to assess the impact of color on each individual group. The box plots in this case also didn't show strong evidence , atleast in most of the groupings, that colorless diamonds should cost higher than the more colored ones. This maybe because variables other than clarity and color might be influencing the price.
...
ggplot(d, aes(color, log10(price), fill=color))+
  geom_boxplot()+
  facet_wrap(~clarity)
  labs(x="Color", y="log10(Price)", title="log10(Price) vs Color grouped by clarity")
```
Then to find a correlation between color and clarity we also created proportion bar chart. This chart will help us answer questions such as does a particular clarity occur more often for a particular color in our data set?. In the below color and clarity plot, we can interestingly see that the best clarity, which is FL, only occurs for D and F grade diamonds in our data set. 
